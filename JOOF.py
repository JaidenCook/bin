...
def poly_fit_plot(Nu_norm,Beam_vec,poly_co_list,k_vec,y_errors=None,Zen=None,theta=None,radec=False,plot=False):
	...
	for index in order_index_list:
		...
		plt.ylabel(r'$\rm{log_{10}(S_b)}$',fontsize = 14)
		plt.legend()
		...
		plt.ylabel(r'$\rm{\chi^2}$',fontsize = 14)
...
def OOF_plot(k_max,OOF_ar,Zen=None,theta=None,plot=False):
	...
	if Zen != None and theta != None:
		...
		plt.ylabel(r'$\rm{log_{10}(OOF(k))}$',fontsize=14)
...
def OOF_analysis_plots(Zen_samp,Az_samp,Ord_list,resid_list,plot=False):
	...
	textstr_X = '\n'.join((r'mode1=%.2f' % (np.float(stats.mode(Ord_list)[0]), ),r'$\mu_{1}=%.2f$' % (np.mean(Ord_list), )))
	textstr_Y = '\n'.join((r'$\mu=%.5f$' % (np.mean(resid_list), ),r'$\sigma=%.5f$' % (np.std(resid_list), )))
	...
	c = ax1.scatter(Az_samp,r,c=Ord_list,label='Order')
	fig.colorbar(c)
	
	ax2 = fig.add_subplot(222, projection='polar')
	ax2.set_title('Polar Residual Map')
	c = ax2.scatter(Az_samp,r,c=np.log10(resid_list),label='Residuals')
	fig.colorbar(c)
	
	ax3 = fig.add_subplot(223)
	ax3.hist(Ord_list,bins=range(24),edgecolor='k',color='g',label='OOF k min',align='left')
	ax3.set_xlabel("Order",fontsize=14)
	...
	ax4.hist(resid_list,align='left',bins=25,range=(np.min(resid_list),2*np.mean(resid_list)),edgecolor='k',color='g',label='Residuals')
	ax4.set_xlabel("Residuals",fontsize=14)
	ax4.set_title('Residual Hist')
	ax4.text(0.75, 0.95, textstr_Y, transform=ax4.transAxes, fontsize=14,verticalalignment='top', bbox=props)
