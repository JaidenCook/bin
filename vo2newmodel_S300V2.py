...
parser.add_option('--qcol',type="string", dest="qcol",
                    help="The name of the spectral curvature q-term column (default=q).", default="q")
...
,data[options.alphacol],data[options.qcol],shape)
...
with open(options.output,"a") as f:
    for Name,RA,Dec,a,b,pa,flux,alpha,q,shape in bigzip:
        RA = replace(RA,":","h",1)
        RA = replace(RA,":","m",1)
        RA = RA+"s"
        Dec = replace(Dec,":","d",1)
        ...
        Dec = Dec+"s"
        if shape=="gaussian":
            f.write(gformatter.format(Name=Name,RA=RA,Dec=Dec,a=a,b=b,pa=pa,flux=flux,\
alpha=alpha,beta=q,freq=options.freq,shape=shape))
        elif shape=="point":
            f.write(pformatter.format(Name=Name,RA=RA,Dec=Dec,flux=flux,alpha=alpha,\
beta=q,freq=options.freq,shape=shape))
