...
poly_co_arr = poly_coefficients(Nu/cent_freq,flat_beam_cube,k_max,True)
...
for j in range(k_max):
	...
	poly_ord_arr[j,:,:] = poly_calc(Nu/cent_freq,poly_co_arr[j,:,:],True)
...
# Since we only fit two orders.
source_poly_co_ar[:,k_max-2] = alpha
source_poly_co_ar[:,k_max-3] = q_curve
...
newvot = Table( [name, ra_str, dec_str, ra, dec, alpha, S_centralfreq, beamvalue, beamvalue_approx, S_centralfreq_uncorrected,\
