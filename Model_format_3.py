...
S_centralfreq = 10**(C1 + C2*(np.log10(cent_freq/300e+6)) + C3*(np.log10(cent_freq/300e+6))**2)

alpha = C2 + 2*C3*np.log10(cent_freq/300e+6)
...
#ord_ar = OOF(Nu/cent_freq,np.transpose(flat_beam_cube),poly_ord_arr)

#OOF(Nu,Beam_arr,px_arr,figcond=False,Zen=None,theta=None,poly_co_list=None)
...
#app_poly_co_ar = np.around((temp_poly_co[:,:k_max-1] + source_poly_co_ar),decimals=2)
app_poly_co_ar = np.zeros(np.shape(temp_poly_co))

#print "Shape Arr1 = {0}, Shape Arr2 = {1}".format(np.shape(temp_poly_co),np.shape(app_poly_co_ar))

app_poly_co_ar[:,len(coeff_arr[0,:])-k_max:] = np.around((temp_poly_co[:,len(coeff_arr[0,:])-k_max:] + coeff_arr[:,::-1]),decimals=2)

#print coeff_arr[:,::-1][:5,:],'\n'
#print temp_poly_co[:5,len(coeff_arr[0,:])-k_max:],'\n'
#print np.around(coeff_arr[:,::-1][:5,:] + temp_poly_co[:5,len(coeff_arr[0,:])-k_max:],2),'\n'
#print app_poly_co_ar[:5,len(coeff_arr[0,:])-k_max:]

app_poly_co_tup_list = [tuple(app_poly_co_ar[l,:]) for l in range(N_sources)]

"""
#### Testing the sensibility of through plotting the result ####
fig = plt.figure()#figsize = (12.5,9.5), dpi=90)

r = np.sin(Zen0_samp[0])
